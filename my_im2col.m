...
temp = [1:slidingDis:size(idxMat,2)];
idxMat([temp temp(end)+1:size(idxMat,2)],[temp temp(end)+1:size(idxMat,2)]) = 1; % take blocks in distances of 'slidingDix', but always take the first and last one (in each row and column).
